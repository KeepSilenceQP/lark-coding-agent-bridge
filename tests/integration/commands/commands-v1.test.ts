...
describe('Bridge command contracts', () => {
  ...
  it('dispatches /project bootstrap bridge commands with metadata and clean slash commands', async () => {
    ...
    const cloudMeta = textMessages.find((text) => text.includes('task_id: project-bootstrap-repo-one-HistoryRedactedBot2'));
    const targetMeta = textMessages.find((text) => text.includes('task_id: project-bootstrap-repo-one-HistoryRedactedBot1'));
    expect(cloudMeta).toContain('<at user_id="ou-cloud-cz">HistoryRedactedBot2</at>');
    expect(cloudMeta).toContain('target_bot: HistoryRedactedBot1');
    expect(cloudMeta).not.toContain('Implementation prompt');
    expect(targetMeta).toContain('<at user_id="ou-live-c">HistoryRedactedBot1</at>');
  ...
  it('pins verified bootstrap receipts and blocks changed live identity later', async () => {
    const h = await createHarness();
    configureSingleBridgeBotBootstrap(h, 'HistoryRedactedBot1', 'ou-live-old', 'repo-pin');

    await expect(
      h.run('/project bootstrap repo-pin HistoryRedactedBot1', {
        chatId: 'oc-project',
        scope: 'oc-project',
        chatMode: 'group',
      }),
    ...
    tryIngestBootstrapReceipt(
      'ou-live-old',
      [
        'task_id: project-bootstrap-repo-pin-HistoryRedactedBot1',
        '/cd → ok',
        '/invite group → ok',
      ].join('\n'),
      [mention('ou-self', 'HistoryRedactedBot4')],
      'ou-self',
    );

    configureSingleBridgeBotBootstrap(h, 'HistoryRedactedBot1', 'ou-live-new', 'repo-pin');
    await expect(
      h.run('/project bootstrap repo-pin HistoryRedactedBot1', {
        chatId: 'oc-project',
        scope: 'oc-project',
