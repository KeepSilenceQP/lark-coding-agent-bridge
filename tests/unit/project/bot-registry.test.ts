...
describe('registry matching', () => {
  ...
  it('matches by canonical name (NFC exact)', () => {
    const result = matchRegistry('HistoryRedactedBot1', registry);
    expect('entry' in result).toBe(true);
    if ('entry' in result) {
      expect(result.entry.canonicalName).toBe('HistoryRedactedBot1');
  ...
  it('prefers canonical over alias match', () => {
    // "HistoryRedactedBot4" is canonical; should match directly
    const result = matchRegistry('HistoryRedactedBot4', registry);
    expect('entry' in result).toBe(true);
    if ('entry' in result) {
      expect(result.entry.canonicalName).toBe('HistoryRedactedBot4');
    }
  });

  it('uses the current fork repository name for local bridge bots', () => {
    const xiaoC = registry.find((entry) => entry.canonicalName === 'HistoryRedactedBot1');
    const xiaoP = registry.find((entry) => entry.canonicalName === 'HistoryRedactedBot4');
...
describe('workspace path resolution', () => {
  ...
  it('falls back to devbox when no local machine', () => {
    const entry: BotRegistryEntry = {
      ...
      aliases: [],
      role: 'bridge',
      machines: [{ kind: 'devbox', root: '/redacted/history/machine-2' }],
      projectRoot: 'my-project',
    };
    const ws = resolveWorkspacePath(entry);
    expect(ws).toBeDefined();
    expect(ws!.path).toBe('/redacted/history/machine-2/my-project');
...
describe('pin-on-first-verify', () => {
  ...
  it('returns no_pin when no binding exists', () => {
    const result = checkPinnedIdentity('HistoryRedactedBot1', 'ou_abc', pins);
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.reason).toBe('no_pin');
  });

  it('returns ok when live openId matches pinned', () => {
    pinBinding('HistoryRedactedBot1', 'ou_abc', 'profile-1', pins);
    const result = checkPinnedIdentity('HistoryRedactedBot1', 'ou_abc', pins);
  ...
  it('returns identity_changed when live openId differs from pinned', () => {
    pinBinding('HistoryRedactedBot1', 'ou_abc', 'profile-1', pins);
    const result = checkPinnedIdentity('HistoryRedactedBot1', 'ou_xyz', pins);
    expect(result.ok).toBe(false);
    if (!result.ok) {
...
describe('merge registry', () => {
  it('overrides default entry by canonicalName', () => {
    ...
    const entry = merged.find((e) => e.canonicalName === 'HistoryRedactedBot1');
