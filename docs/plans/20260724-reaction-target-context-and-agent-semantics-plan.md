...
- **B9（invariants 修复，云上C总 Implementer，第 2 轮 — 待小P Review）**：小P Code Review（e96f1a7..33860fe）BLOCKED 5 项，本轮逐项修复：
  - B1（ordinary workChain 生命周期）：`runAgentBatch` 现为**每个 input unit（ordinary+reaction）**分配/继承 workChain——reaction 经 `consumeReactionTurnMeta`，ordinary 经 `resolveOrAllocate(scope, replyToMessageId)`+`markCurrent`+`acquireUnit`；`_registerOutboundOnStream` 对 ordinary+reaction 均登记 Card/Markdown/CoT/Text 输出；`isReactionTurn` 区分 supersede/tracker 仅对 reaction 生效。
  - B2（stop current-work 覆盖 queued+reserved+active）：stop handler 改用 `activeRuns.hasActiveOrReserved(scope) || pending.pendingCount(scope) > 0`（新增 `ActiveRuns.hasActiveOrReserved`/`hasReservation`），不再只查 active handle——queued/reserved reaction 不再误报 no-work。
  - B3（rev2 context 不被 evict 删除）：buffer flush handler 改为**先 evict old（`evictInFlightReactionEntry`，含 `contextStore.delete`）再写 rev2 context/meta/tracker/barrier**——evict-old-before-write-rev2，rev2 context 存活。
  - B4（per-unit in-flight，替代 chain 级 terminal）：`WorkChainStore` 新增 `acquireUnit(chainId, unitId)`/`releaseUnit(chainId, unitId)` + `inFlightUnits` 集；`finalizeReactionRun` 改用 `releaseUnit`（最后一个 unit terminal 才 `markTerminal`），覆盖同 chain 不同 key/sibling + error/catch 路径（catch 也 finalize）。
  - B5（测试+Plan）：新增 `WorkChainStore.acquireUnit/releaseUnit`（B4 per-unit sibling）、`ActiveRuns.hasActiveOrReserved`（B2 reservation）、`isLatest` gate（rev1 terminal 不清 rev2）测试；typecheck=0、build OK、diff-check clean、reaction+runtime+executor 361/361 通过。完整 ordinary-long-task→stop / CoT-resolve-during-streaming / rev1→rev2 prompt-only-latest 端到端 production-seam 测试仍建议跟进（核心逻辑已单测+生产 wiring 已接）。
