...
Plan Reviewer: independent SubAgent（Amendment exact-range review）；小P只评估和采纳 findings
...
- `src/bot/reaction/work-chain.ts` 当前只有 `outboundMap/registerOutbound/resolveCurrentChain`，没有 inbound trigger map；`PendingQueue` regular unit 可以合并多条 `messages[]`，但没有把每个真实 message ID 关联到该 unit 的 lease/workChain。
- 当前 stop caller 先判 `hasWork`，再查 `resolveCurrentChain(targetMessageId)`；这对 unknown user target 会产生可见 no-work/fail-closed，与已确认 Spec 的 user eligibility-before-no-work 冲突。
- 当前 control ledger removed 路径已有持久 added 匹配能力，可作为 user-target removed 绕过 own-message 的唯一依据；不得依赖已可能过期的运行期 trigger correlation。
...
命中即静默丢弃：不读写用户 snapshot、不更新 revision、不排队、不回复。`reconciler` 从 `messageReaction.list` 重建 `effectiveReactionSet` 时，剔除 `operator_type==='app'` 或 operator==self 的项，保证 Bridge 自己加/删 `Typing` 等工作态 Reaction 不反向触发 Agent。现有 self-**message** 过滤（`channel.ts:946-961`）对普通语义保留为「仅消费本 Bot 消息上的 Reaction」的路由前提。`stop_current_work` 是唯一例外：added 仅可在运行期证明目标为本进程登记的 current/historical inbound trigger 后绕过；removed 仅可在持久 control ledger 命中同 operator/target/emoji 的已消费 added 后绕过。用户消息上的肯定、解释、完成及未映射 Reaction 仍静默丢弃。
...
任一门禁失败：静默拒绝——不更新 revision/快照、不入 pending、不启动 Agent、不 stop/interrupt、不取消队列、不发非授权提示。日志仅记受限 scope、operator 后缀、reason。`stop_current_work` 必须先通过**完全相同的 Reaction 无 @ 门禁**才进控制面：固定 `mentionedBot=false`；不得因为原 trigger 消息曾经 @ Bot、目标消息作者、或 `/stop` 命令特例而取得额外资格。它与 `/stop` 只共享通过门禁后的 interrupt + cancel 效果。
...
- `work-chain.ts` 维护有界运行期 `workChainId` 关联，**不交给模型推断**。
  ...
  - regular PendingUnit 可能合并同 debounce window 的多条普通消息；必须把该 unit 中**全部真实、用户可见的 inbound message ID**登记为 `triggerMessageIds[] → workChainId`。不能只登记 first/last message，不能用 synthetic unit ID 替代用户可添加 Reaction 的消息 ID。纯 synthetic card callback 不创建 user-trigger stop 入口。
...
`stop_current_work` 的 added 与 removed 都不依赖「普通 Reaction → buffer → pending → Agent run」链路。`control-ledger.ts` 维护独立、持久控制事件 ledger（同 DD6 模式）。合法 target class 只有：(a) Bot outbound message/card；(b) 触发 queued/reserved/active input unit 的真实 inbound user message。后者是流式卡片更新期间的主 stop 入口，仅 `No` / `CrossMark` / `MinusOne` 可用。

- **added**：完成 self-operator（DD2）、安全路由、target sender 分类、Reaction 无 @ 权限门禁（DD3）、语义映射（DD4）与控制事件防重后，**先做 target eligibility，再判断 work 状态**（**不**等普通 quiet window，**不**与紧随其后的 removed 相消）：
  1. user target 从未登记、已过期/LRU 淘汰或重启后 correlation 丢失 → 静默丢弃，不回复、不写 control ledger；用户消息上的非停止 Reaction 同样静默丢弃。
  2. eligible target 是 retained historical user trigger，且 scope 完全无 work → 幂等回复「当前没有需要停止的任务」并持久标记已消费；若 scope 存在另一 current chain → fail closed，回复未停止及 `/stop` 提示并持久标记已消费。
  3. Bot target 沿用已有规则：scope 完全无 work时可见回复无任务；有 work 时，unknown/historical/unrelated mapping 可见 fail closed。
  4. target 是 current chain 的 inbound trigger 或 Bot outbound → 立即执行 `activeRuns.interrupt(scope)` + `pending.cancel(scope)`，取消该 scope 全部 queued sibling unit，使流式输出收敛为 interrupted；持久标记 stop-added 已消费并由 Bridge 回复可见停止结果。stop 不启动 Agent。
- **removed**：先通过 self-operator、安全路由、target sender 分类和 Reaction 无 @ 权限门禁，不要求 chain 仍 current/historical，也不依赖运行期 correlation。对于 user target，只有持久 control ledger 命中同 operator/target/emoji 的已消费 added 才可绕过 own-message filter；匹配后只回复一次「撤回停止 Reaction 不会自动恢复工作」并持久标记 removed。无匹配 added → 静默 no-op。
- added/removed 都优先用 reaction/event 可用稳定 ID 生成防重 fingerprint；缺稳定 ID 时用规范化事件字段 + action time。重复投递/乱序/重启后已消费事件 = 静默 no-op。
- 权限判断是 Reaction 自身的 `canUse* + mentionedBot=false decideGroupResponse`；只有控制效果与当前 scope `/stop` 一致。`workChainId` 仅用于证明 target eligibility/关联并防历史或无关目标误触发，**不**把 `/stop` 改造成局部取消。
...
- [ ] RED/回归至少覆盖：
  ...
  - Bot target 的 no-work、historical/unrelated fail-closed、current stop 行为无回归；
  - 流式卡片仍在更新时，对触发它的用户消息添加 stop Reaction，证明自然完成前 interrupt 且无后继 Agent run。
...
- Depends: Amendment Code Review Gate 通过。
...
Progress update (2026-07-24): terminal removal live retry passed after B8/R8,
but stop live exposed the product/runtime gap now captured by Spec `526cbcb`.
An updating streaming card is not a usable reaction surface; adding `No` to
the user message that triggered the active task was silently dropped because
the pipeline applies own-message filtering before stop classification and no
inbound-trigger correlation exists. Unit 11 stop is frozen until Unit 12,
independent Code Review, redeploy, restart readback, and the amended live
oracle pass. Previously passed approve/explain/done/unmapped/removed evidence
remains historical evidence and is not reinterpreted as stop completion.
...
| 流式卡片更新中，对触发当前任务的用户消息 stop | 12、11 |
| regular PendingUnit 全部真实 inbound message ID 映射同一 chain | 12 |
| user trigger 上非停止 Reaction 静默过滤 | 12 |
| unknown/expired/LRU/restart-lost user target 静默且不写 ledger | 12 |
| retained historical user trigger：无 work / 另一 current chain | 12 |
| current trigger mapping 超 TTL/cap 不淘汰，重新 current 恢复保护 | 12 |
| user-target stop removed 仅凭 consumed-added ledger，跨 terminal/淘汰/重启 | 12 |
| trigger 作者无权限、另一 operator 仅按 Reaction 无 @ 门禁 | 12 |
| scope/thread 与 trigger correlation 不一致 | 12 |
| Bot outbound stop no-work/fail-closed/current 行为无回归 | 9、12 |
...
- **不得**回退：Reaction 权限门禁（DD3）、self-operator guard（DD2）、安全路由/target sender 分类、普通语义 own-message 过滤与 stop 的受限 user-trigger 例外、飞书回复引用关系（`replyTo` 目标消息）。
...
- **RD9（Spec amendment `526cbcb`）**：流式卡片编辑中不要求其自身可添加 Reaction；`stop_current_work` 额外接受本进程登记的真实 inbound trigger user message。user target 必须 eligibility-before-no-work，unknown/expired/restart-lost 静默；removed 仅凭持久 consumed-added ledger；普通非停止 Reaction 仍只消费 Bot target。
