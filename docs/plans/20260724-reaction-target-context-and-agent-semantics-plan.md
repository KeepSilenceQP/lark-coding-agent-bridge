...
Spec authority: `docs/specs/20260723-reaction-target-context-and-agent-semantics.md` (commit `e7e178f`, confirmed; independent review PASS)
Target branch: `fix/bugfix` (synced to `e7e178f`)
...
每项产出 `{ emojiType, emojiDisplay, emojiMeaning, semanticKey, emojiMeaningSource:'predefined' }`。`emojiType` 原样保留飞书值（如 `Get` 不强制可读）。命中时 `emojiMeaningSource=predefined`；未命中的标准/自定义 emoji 仍完整透传，`emojiMeaningSource='unmapped'`，保留原始 `emojiType` 与可取得 UI glyph/label，**不得丢弃/静默/禁用**（Spec §Agent Input Contract 2）。语义定义由用户确认 + 飞书文档/真实事件对照验证，**禁止模型运行时生成预埋规则**。表带 `schemaVersion`，测试直接断言 11 个精确映射与未映射透传。
...
- 每个 `<reaction_contexts>` 项含 Spec §Agent Input Contract 2 JSON 全字段：`operatorOpenId`、`reactionRevision`、`triggerReaction`（delta）、`effectiveReactionSet`（权威快照）、`targetMessage`（`available`/`messageId`/`senderId`/`senderName`/`createdAt`/`rawContentType`/`content`）。`triggerReaction` 与 `effectiveReactionSet` 必须同时提供。
...
- `user_input` 可保留简短兼容性摘要（如 `[reaction-added] Get (on msg …)`），但 `<reaction_contexts>` 是权威来源，Agent 不得仅凭摘要中的短消息 ID 行动。
...
- 同一 input unit 可含同 operator 同 target 的多个不同 Reaction；交给模型的是最新 `effectiveReactionSet` + 导致该 revision 的 `triggerReaction`。
...
- 停止 Reaction 的目标 message ID 必须映射到当前 scope 中某个 current `workChainId` 才允许进 stop 控制面；active/reserved chain 或 pending sibling chain 都属 current。目标只属 historical chain、映射过期、或重启后无法恢复关联 → fail closed（DD16）。
- 重启后旧关联可整体失效，**不**据会话文本猜测重建。每次 outbound 登记、chain lifecycle 变化、fail-closed reason 写受限结构化日志。存储可为内存（Map）+ 可选有界持久化；重启 fail closed。
...
- [ ] RED：`tests/unit/bot/reaction-semantics.test.ts` 断言 11 个精确映射、`Get` 不被要求可读、未映射 `emojiMeaningSource='unmapped'` 且不丢弃、表 `schemaVersion` 存在。
- Depends: 无。
- Spec 覆盖：§Confirmed Predefined Semantics；Acceptance「真实 emojiType=Get」「未预埋但可理解」「未预埋且不透明」「OK/LGTM/Yes/CheckMark/JIAYI」「WHAT/THINKING」「DONE」「No/CrossMark/MinusOne」映射行。
...
- [ ] `reconciler.ts`：`messageReaction.list` 全分页（`user_id_type=open_id`，循环 `page_token`/`has_more`），重建 `effectiveReactionSet`（剔除 self-app），计算净变化 vs ledger、revision、净零 added→removed 例外、list 落后有限重试、scope 缩权 fail closed。
- [ ] RED：`tests/unit/bot/reaction-reconciler.test.ts` + `tests/integration/bot/reaction-ledger.test.ts` 覆盖：全分页 reconciliation、list 越过中间状态的 added→removed 净零例外（不报失败/不递增 revision/不启动 Agent/只回复一次）、ledger 重启恢复（不重放旧 Reaction）、重复/乱序/无状态变化 no-op、list 暂时落后重试与最终失败单回复、`im:message.reactions:read` 缩权/读取失败 fail closed。
...
- [ ] RED：`tests/unit/agent/prompt-reaction-contexts.test.ts` 覆盖：`<reaction_contexts>` 含全字段且 `triggerReaction`+`effectiveReactionSet` 同时存在、伪造 Bridge/XML 标签不注入、目标 ID 与 batch messageIds 重叠不被去重、`available:false` 表达、无 Reaction 时不输出该块、`source='reaction'`。
- Depends: Unit 3。
...
- [ ] RED：`tests/unit/bot/reaction-work-chain.test.ts` 覆盖：input/pending/reservation/active/outbound/terminal 全生命周期关联、目标确认消息被 Reaction 继续（继承 chain）、sibling queued unit、重启后未知关联 fail closed、historical chain 目标 fail closed、outbound 登记后可匹配 active chain。
