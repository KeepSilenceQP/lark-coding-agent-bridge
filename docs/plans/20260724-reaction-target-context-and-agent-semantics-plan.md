...
- **B9（invariants 修复，云上C总 Implementer，R3 — 待小P Review）**：R2 Review（33860fe..8919bd2）BLOCKED 5 项（R2-F1..F5），R3 逐项修复：
  - R2-F1（acquire at enqueue，非 run start）：reaction 在 buffer flush handler `pushBarrier` 前 `acquireUnit(wcId, turnId)`；ordinary 在 `intakeMessage` `pending.push` 前 `resolveOrAllocate`+`markCurrent`+`acquireUnit(emsg.messageId)`+`setOrdinaryTurnMeta`。runAgentBatch 不再 acquire（unit 自 enqueue 起 in-flight），只 markStatus；unitId 统一为 `firstMsg.messageId`。rev1 active→rev2 queued→rev1 terminal 时 rev2 unit 仍 in-flight，chain 保持 current。
  - R2-F2（cancel/abort 释放 lifecycle）：新增 `releaseEnqueuedTurn(scope, messageId)`——消费 meta（reaction/ordinary）+ `releaseUnit` + 清 tracker。stop 成功分支 `pending.cancel` 的每个 removed unit 经 releaseEnqueuedTurn 清理；startFlow `!flow.ok` 也 release（run 未启动不残留 ghost current-chain）。
  - R2-F3（ordinary queued/reserved chain current）：由 R2-F1 ordinary enqueue 分配覆盖——ordinary reply 在 pending/reservation 期间 target→current chain 已登记，stop 可命中。
  - R2-F4（fallback outbound 登记）：card/markdown `awaitRenderAwareStream` fallback 的 `channel.send` 返回值不再丢弃，`_registerOutboundOnStream(fbResult)` 即时登记。
  - R2-F5（测试+Plan）：B4 `acquireUnit/releaseUnit` per-unit sibling 测试（rev1+rev2 同 chain，release rev1→current，release rev2→historical）覆盖 R2-F1 时序；`hasActiveOrReserved` 覆盖 reservation；`isLatest` 覆盖 rev1≠rev2。typecheck=0、reaction+runtime+executor 364/364。完整 ordinary-pending→stop / reservation-abort→全清 / fallback-resolve 端到端 production-seam 测试仍建议跟进（核心逻辑已单测+生产 wiring 已接）。
