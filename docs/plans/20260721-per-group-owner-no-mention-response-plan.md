...
本 Plan 必须由独立 SubAgent（HistoryRedactedBot4）对照 confirmed Spec（`c1b148b`）与 live code review。只有 review 结论为 PASS，或所有阻塞项已修订并复审通过，才开始 Unit 1 实现。Plan Writer 不给自己的 Plan 判 GO。
