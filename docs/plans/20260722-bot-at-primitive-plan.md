...
- a second three-platform variant makes the wrapper exit immediately while the heartbeat child retains inherited stdout/stderr. POSIX must still kill the surviving process group and close boundedly. On Windows, Node closes inherited pipes on exit so the fixture proves child survival + out-of-band kill + no residue; the runner's close returns `exit` per the [Windows observational boundary](#windows-observational-boundary) (mock tests cover `hasPid=false→termination-unconfirmed` separately). The test kills its known fixture child out of band and proves the test suite leaves no process behind;
...
<a id="windows-observational-boundary"></a>

**Windows observational boundary**: Node on Windows closes inherited pipes on `process.exit()`, so the held-pipe early-exit scenario (wrapper exits before timeout, child holds pipe, runner timer kills orphan) cannot be reproduced at the process-tree level. The runner’s `close` event fires on the normal `exit→close` lifecycle; at close time there is no portable signal to distinguish "normal completion with no orphans" from "wrapper exited early leaving live descendants." The Windows early-exit real fixture therefore accepts `settled=exit` as the correct runner behaviour on this platform. The fixture proves child survival *independently* of runner pipes through a detached heartbeat-file mechanism: (a) the child is spawned with `detached: true` and `stdio: 'ignore'`, fully decoupled from wrapper lifecycle; (b) the child writes a heartbeat counter to a known file every ~80ms; (c) after runner settles, the test verifies the child PID is alive via signal 0, reads the heartbeat counter, waits 200ms, and verifies the counter advanced — proving the child was actively executing after wrapper exit; (d) the test then kills the child via `taskkill /T /F` in `finally` and asserts the child is dead and no residue remains. The mock tests cover the `hasPid=false → killTree → termination-unconfirmed` contract separately. This boundary must be re-evaluated if a future Node or Windows version provides reliable orphan detection at close time.
...
- On Windows, if the `lark-cli` wrapper exits before `close` while an orphan request process still owns inherited pipes, plain `taskkill /T` can no longer use the extinct wrapper PID as a tree root (timer-not-fired path: `hasPid=false → killTree → termination-unconfirmed`). If the wrapper exits and `close` fires before the timer (normal `exit→close` lifecycle, e.g. early-exit with Node pipe cleanup), the runner returns `exit` — an acknowledged observational boundary on this platform (see [Windows observational boundary](#windows-observational-boundary)). The `runAtBot` layer's strict envelope/message_id validation provides the fail-safe. v1 forbids automatic retry; adding Job Object/native supervisor ownership is deferred rather than hidden behind a false guarantee.
...
- **R7 `BLOCKED`** *(superseded by R12 Windows observational boundary)*: Windows `taskkill /T` cannot use an already-exited wrapper PID to discover orphan descendants. The Plan does not add Job Objects or a native supervisor: Windows uses taskkill only while the wrapper is live. The mock `hasPid=false → killTree → termination-unconfirmed` contract is covered separately. Per R12 observational boundary, the real Windows early-exit fixture now accepts `settled=exit` (Node closes inherited pipes on `process.exit()`, runner cannot distinguish orphan survival at close time) and instead proves child survival + out-of-band kill + no residue.
- **R8 `PASS`** *(superseded by R12 Windows observational boundary)*: independent review found no remaining P0/P1/P2. The Reviewer accepted the explicit Windows `termination-unconfirmed` boundary as faithful to the Spec’s nonzero/no-false-success/no-auto-retry contract. Subsequently revised in R12: the observational boundary was found to be a `close=exit` path rather than `termination-unconfirmed` at the runner level, with the same fail-safe semantics enforced at the `runAtBot` layer through strict envelope/message_id validation.
- **R11–R13 `CONDITIONAL`**: CI Gate revisions. R11 introduced production close=termination-unconfirmed on Windows hasPid=false (reverted in R12 P0). R12 established the Windows observational boundary (Node closes inherited pipes on exit, runner returns exit on normal close). R13 resolved Plan consistency and CI concurrency.
- **R14–R16 `CONDITIONAL`**: Windows child survival evidence. R14 added detached:true to child spawn. R15 introduced heartbeat-file counter to prove continued execution independent of runner pipes. R16 fixed wrapper.pid write order (first), pre-built childCode to avoid -e escaping, added exitCode diagnostics and wpDead assertion; running.
