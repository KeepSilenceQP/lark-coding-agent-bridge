...
UI 继续引用被 Reaction 的原消息。普通语义仍只处理当前 Bot 自己消息上的 Reaction；
`stop_current_work` 是控制面例外：除了当前 chain 中可添加 Reaction 的 Bot 消息外，也
允许作用于**触发当前 queued/reserved/active work 的入站用户消息**。这是流式卡片在更新
期间不能添加 Reaction、但用户仍需在任务完成前表达 stop 的必要入口；非停止 Reaction
不得因此扩展到用户消息。
...
live 验收确认：流式卡片在持续更新期间无法由用户添加 Reaction，更新完成后才可添加。
因此“对正在输出的卡片添加 `No`”不能作为 active-run stop 的唯一入口。Bridge 必须把
触发当前 input unit 的入站消息 ID 纳入 `workChainId` correlation，使用户可在原始 trigger
消息上添加停止 Reaction；这不要求改变飞书卡片能力，也不把普通用户消息上的其他
Reaction 交给 Agent。

本需求只修复 Reaction 的访问门禁、Agent 上下文、大模型处理规则和停止控制关联，不改变飞书事件订阅与平台 scope、普通消息/评论/卡片回调路由、既有群响应策略本身、会话隔离或回复 UI。
...
| 用户给其他 Bot 或普通用户消息点非停止 Reaction | 当前 Bot 不启动 Agent run；仅 current trigger 上的停止 Reaction 是控制面例外 |
...
每条验收保存实际动态 prompt、共享 System Prompt 版本、工具调用、可观察副作用、最终回复和飞书消息 ID。实机至少覆盖四个预埋 `semanticKey` 各一个代表 emoji、一次未预埋 Reaction 和一次 removed，核对 UI 引用、Agent 输入、System Prompt、工具副作用与最终行为一致；停止场景必须在流式卡片仍处于更新状态时，对触发当前任务的入站用户消息添加代表性 stop Reaction，证明 interrupt 发生在旧 run 完成之前，且没有后继 Agent run 自动启动。另需验证相同用户消息上的非停止 Reaction 不被消费。

## Next Phase

本次 stop trigger-message amendment 独立 Review 通过后再修订 Coding Plan。Plan 必须把“权限与 self-operator 门禁”“Reaction event buffer/权威快照/持久 ledger/revision”“动态 Reaction 上下文”“共享 Bridge System Prompt 规则”“全部有效 Reaction 的可见回复”和“停止 Reaction 控制面”作为同一修复的必做单元，并覆盖普通消息权限函数复用、四种群响应模式、所有授权 operator 共用语义、11 个已确认 emoji 映射、未映射 Reaction 透传、`rawClient.im.v1.messageReaction.list` 全分页与 scope、净零 added→removed 例外、重复/乱序/重启幂等、in-flight revision 失效与替代、superseded 流式回复、terminal 后撤回的 Bridge 回复、Reaction 元数据在 pending batch 中的表示、目标消息规范化复用、inbound trigger 与 Bot outbound 两类 `workChainId` 停止目标关联、用户消息仅 stop 例外、stop added/removed 独立控制 ledger 与快路径、现有 `/stop`/ActiveRuns/PendingQueue 能力复用、两种 Agent 的 Prompt 注入、模型行为对照验收及实机验收；Plan 修订评审通过前不修改运行代码。
