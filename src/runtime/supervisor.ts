  ...
  async bringUp(nowIso: string): Promise<void> {
    ...
    try {
      ...
      this.bridge = await this.startChannelFn({
        ...
        promptSessionService: this.promptSessionService,
    ...
    const currentControls: Controls = {
      ...
      async refreshOwner(channelOverride) {
        ...
        try {
          ...
          if (stored) currentControls.profileConfig.access.owner = { appId, openId: ownerId };
        } catch (err) {
...
export class Supervisor {
  ...
  async startProfile(profile: string): Promise<void> {
    ...
    if (promptSessionService.health.health === 'healthy') {
      await promptSessionService.gc().catch((err) =>
        log.warn('prompt-session', 'gc-failed', {
          profile: appPaths.profile,
          err: String(err),
        }),
      );
