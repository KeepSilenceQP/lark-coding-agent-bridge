...
export function buildConfigView(state: MutableProfileState, live = false): ConfigView {
  ...
  return {
    ...
    groupResponseMode: state.profileConfig.access.groupResponseMode,
...
function parseConfigBody(state: MutableProfileState, body: unknown): ParsedConfig {
  ...
  const groupResponseMode: GroupResponseMode =
    typeof fv.requireMentionInGroup === 'boolean'
      ? (fv.requireMentionInGroup ? 'mention-only' : 'all-messages')
      : state.profileConfig.access.groupResponseMode;
...
export async function applyConfig(rt: UiRuntime, body: unknown): Promise<ConfigView> {
  ...
  try {
    ...
    await savePreferencesConfig(rt, p.nextPreferences, p.groupResponseMode, p.larkCliIdentity, p.mode);
  } catch (err) {
    if (identityApplied) {
...
export async function applyConfigToDisk(
  ...
  try {
    await savePreferencesConfig(state, p.nextPreferences, p.groupResponseMode, p.larkCliIdentity, p.mode);
  } catch (err) {
    throw new ApiError(500, `保存失败：${err instanceof Error ? err.message : String(err)}`);
  }
